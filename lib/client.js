...
Client.prototype._request = function(opts, callback, state) {
  var self = this;
  ...
  if (opts.format) {
    var format = Array.isArray(opts.format) ? opts.format : [opts.format];

    finish = function() {
      var args = Array.prototype.slice.call(arguments);

      args.set = utils.setArray;

      format.some(function(fn) { return fn(args); });
  ...
  ctx.retry = function() {
    ...
    self._request(opts, callback, state);
  };
