...
Client.prototype._encode = function(mime, value, buffer) {
  if (!this._opts.encoders[mime]) {
    throw new Error('unknown encoder: ' + mime);
  }

  try {
    return this._opts.encoders[mime](value, buffer);
  } catch (err) {
...
Client.prototype.__pushExts = function(stack, name, opts) {
  if (this._exts[name]) {
    stack.push.apply(stack, this._exts[name]);
  }

  if (opts && opts.exts && opts.exts[name]) {
    stack.push.apply(stack, opts.exts[name]);
  }
};
...
Client.prototype._request = function(opts) {
  ...
  if (this.__rapi) {
    ...
    if (ctx.args.length > 1) {
      ctx.callback = ctx.args[ctx.args.length - 1];
    }
    ...
    if (ctx.client._exts.onReturn) {
      ctx.client._exts.onReturn.forEach(function(fn) {
        result = fn(ctx, result);
      });
    }

    if (typeof ctx.callback !== 'function') {
      ctx.client.emit('error', ctx.client.__err('callback required', opts));
      return result;
    }
    ...
    if (typeof opts.options === 'function') {
      try {
        ...
        ctx.callback(ctx.client.__err(err, opts));
        return result;
      }
    }
    ...
    // restart request
    ctx.retry = function() {
      ctx.client._log(['debug', 'request', 'retry'].concat(opts.tags || []));

      delete ctx.err;
      delete ctx.res;

      ctx.client._request.call({ __rapi: ctx });
    };

    ctx.stack = [];

    ctx.client.__pushExts(ctx.stack, 'onCreate', opts);

    ctx.stack.push(ctx.client.__create);

    ctx.client.__pushExts(ctx.stack, 'onRequest', opts);

    ctx.stack.push(ctx.client.__execute);

    ctx.client.__pushExts(ctx.stack, 'onResponse', opts);

    ctx.stack.push.apply(ctx.stack, ctx.args.slice(1, ctx.args.length - 1));
  }

  var i = 0;
  function next(err) {
    if (err) {
      if (!ctx.next) return ctx.callback(ctx.client.__err(err));
      ctx.err = err;
    }

    if (err === false) {
      return ctx.callback.apply(null, Array.prototype.slice.call(arguments, 1));
    }
    ...
    var fn = ctx.stack[i++];
    if (fn) {
      fn.call(ctx.client, ctx, next);
    } else {
      ctx.callback.call(ctx.client, ctx.client.__err(ctx.err), ctx.res);
    }
  }
...
Client.prototype.__create = function(ctx, next) {
  ...
  // combine client and request headers
  var headers = utils.mergeHeaders(self._opts.headers, opts.headers);

  // encode and set path parameters
  if (!utils.isEmpty(opts.params)) {
    path = path.replace(/\{(\w+)\}/g, function(src, dst) {
      if (opts.params.hasOwnProperty(dst)) {
        ...
        return opts.params[dst] && opts.params[dst].encode === false ?
          opts.params[dst] :
  ...
  // encode and append query parameters
  ...
  // encode body and set content headers
