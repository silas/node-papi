...
Client.prototype.__pushExts = function(ctx, name) {
  if (this._exts[name]) {
...
Client.prototype._request = function(opts) {
  ...
  if (this.__rapi) {
    ...
    if (self._exts.onPreRequestSetup) {
      self._exts.onPreRequestSetup.forEach(function(fn) { fn(ctx); });
    }

    if (typeof ctx._callback !== 'function') {
      self.emit('error', self.__err('callback required',
        opts instanceof Error ? null : opts));
      return ctx._return;
    }

    if (opts instanceof Error) {
      return ctx._callback(self.__err(opts));
    }
    ...
    self.__pushExts(ctx, 'onRequest');

    ctx._stack.push(self.__execute);

    self.__pushExts(ctx, 'onResponse');

    ctx._stack.push.apply(ctx._stack, ctx._args.slice(1, ctx._args.length - 1));

    if (self._exts.onPostRequestSetup) {
      self._exts.onPostRequestSetup.forEach(function(fn) { fn(ctx); });
    }
  }
  ...
  function next(err) {
    if (err) {
      if (!ctx._next) return ctx._callback(self.__err(err, opts));
      ctx.err = err;
    }
    ...
    if (fn) {
      ...
      ctx._callback.call(self, self.__err(ctx.err, opts), ctx.res);
    }
  }

  if (ctx._return) {
    var result = ctx._return;
    delete ctx._return;
