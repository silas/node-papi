...
Client.prototype._request = function() {
  ...
  if (typeof args.opts.validate === 'function') {
    try {
      args.opts.validate.call({
        path: args.path,
        opts: args.opts,
      });
    } catch (err) {
      args.callback(this.__err(err, args.opts));
      return result;
    }
  }
  ...
  ctx.retry = function() {
    ...
    self._request(args.path, args.opts, args.callback);
  };

  var jobs = [];

  self.__pushExts(jobs, 'onCreate', ctx.opts);

  jobs.push(self.__create);

  self.__pushExts(jobs, 'onRequest', ctx.opts);

  jobs.push(self.__execute);

  self.__pushExts(jobs, 'onResponse', ctx.opts);

  var i = 0;
  function next(err) {
    if (err) {
      if (!ctx.next) return args.callback(self.__err(err));
      ctx.err = err;
    }
    ...
    if (fn) {
      ...
      args.callback.call(self, self.__err(ctx.err), ctx.res);
    }
  }
