...
Client.prototype.__create = function(request, next) {
  ...
  if (opts.multipart !== undefined) {
	...
	throw errors.Validation('body specified for multipart POST');
      }
      if (opts.method !==  'POST') {
	throw errors.Validation('invalid method for multipart body: ' +
				opts.method);
      }
      if (opts.type !== undefined && opts.type !== 'multipart') {
	throw errors.Validation('multipart body needs multipart type');
      }
      if (headers['content-type'] !== undefined) {
	throw errors.Validation('content-type specified for multipart body');
      }
      var boundary = Math.random().toString(36).substring(7);
      headers['content-type'] = 'multipart/form-data; boundary=' + boundary;
      request.body = self._encode(
	'multipart/form-data',
	{ parts: opts.multipart, boundary: boundary });
  ...
  if (opts.body !== undefined) {
    ...
    if (isStream) {
      ...
      headers['content-length'] = request.body.length;
	     ...
	     !~constants.EXCLUDE_CONTENT_LENGTH.indexOf(opts.method)) {
