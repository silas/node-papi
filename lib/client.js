...
Client.prototype._request = function(path, opts, callback) {
  ...
  if (self._exts.onPreCreate) {
    jobs = jobs.concat(self._exts.onPreCreate);
  }

  jobs.push(self.__create);

  if (self._exts.onPreExecute) {
    jobs = jobs.concat(self._exts.onPreExecute);
  }

  jobs.push(self.__execute);

  if (self._exts.onPostExecute) {
    jobs = jobs.concat(self._exts.onPostExecute);
  }

  var i = 0;
  (function next(err) {
    if (err) {
      if (!err.next) return callback(err);
      ctx.err = err;
    }
    var fn = jobs[i++];
    if (fn) {
      ...
      callback.call(self, ctx.err, ctx.res);
    }
  })();
};
