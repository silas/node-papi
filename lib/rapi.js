...
Rapi.prototype._log = function(tags) {
...
Rapi.prototype._before = function(ctx, next) {
  next();
};
...
Rapi.prototype._after = function(ctx, next) {
  next();
};
...
Rapi.prototype._request = function(path, opts, callback) {
  var self = this;
...
Rapi.prototype.__request = function(path, opts, callback) {
  var self = this;
  ...
  if (opts.timeout && opts.timeout > 0) {
    timeout = setTimeout(function() {
      request.abort();
      request._log(['error', 'request', 'timeout'].concat(tags));
    }, opts.timeout);
...
METHODS.forEach(function(method) {
  ...
  Rapi.prototype['_' + method] = function(path, opts, callback) {
